...
func (c *Client) Read() {
	...
	for {
		...
		c.testTimeout(ctx, message)
		// 测试 收到消息将消息发送回去
		// c.Message <- message
	}
}
func (c *Client) testTimeout(ctx context.Context, message []byte) {
...
func (c *Client) Write() {
	...
	for {
		...
		case message, ok := <-c.Message:
			...
			log.Printf("client [%s] write message: %v", c.Id, message)
...
// 向指定的 client 发送数据
func (manager *Manager) Send(id string, group string, message Message) {
	data := &MessageData{
		Id:      id,
		Group:   group,
		Message: &message,
...
// 向指定的 Group 广播
func (manager *Manager) SendGroup(group string, message Message) {
...
// 广播
func (manager *Manager) SendAll(message Message) {
...
func (manager *Manager) WsClient(ctx *gin.Context) {
	...
	manager.Send(client.Id, client.Group, Message{
		Code:  "0",
		Data:  "ok",
		Topic: "default",
	})
...
func TestSendGroup() {
	for {
		...
		WebsocketManager.SendGroup("leffss", Message{
			Code:  "0",
			Data:  "SendAll message ----" + time.Now().Format("2006-01-02 15:04:05"),
			Topic: "test",
		})
	}
}
